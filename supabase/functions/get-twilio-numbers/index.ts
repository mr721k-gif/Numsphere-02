...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return new Response(
        JSON.stringify({ error: "Twilio API failed", details: text }),
        {
          status: resp.status,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const formattedNumbers = (data.incoming_phone_numbers || []).map(
      (n: any) => ({
        sid: n.sid,
        ...
        origin: "twilio",
        owned:
          ownedNumbers?.some((owned) => owned.twilio_sid === n.sid) || false,
      }),
    );
